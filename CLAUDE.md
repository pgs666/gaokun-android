...
**当前阶段：Stage 6 M15 进行中 — ★s2idle 大幅收敛：复位发生在【挂起进入】而非唤醒；已找到能绕过复位的 cmdline；"醒不回来"确认是另一个独立问题**（每次开工时更新这一行）
...
>   是目前最安全最快的复现器。
> - ★★ **`pcie_aspm=off nvme_core.default_ps_max_latency_us=0` 让
>   `pm_test=devices` 第一次返回 rc=0** —— 复位被绕过了。
>   随后的真实挂起**睡进去了但没醒** ⇒ **"醒不回来"是另一个独立问题**，
>   被"挂起时复位"盖了整整两轮。
>   ⚠️ 变量还没完全分离：#40 证明**只去掉 `pcie_aspm.policy=powersupersave`
>   没用**（仍复位）；#41（只关 NVMe APST）机器**挂住**，日志未取回。
