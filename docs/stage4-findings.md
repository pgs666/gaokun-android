...
## #44 ★WiFi 在大流量下会塌到 10 KB/s：`ath11k` 的 `msdu_done` 丢帧（2026-08-21）

装 OTA 时撞上的：1.1 GB 的 payload 下到 2% 就几乎停住，**约 10 KB/s**，
而**同一台机器同一个源，二十分钟前刚以约 1.1 MB/s 下完过一次**。
...
|---|---|
| RSSI | **−35 dBm**（极好）|
| 标准 / 频段 | 802.11ax / 5200 MHz |
| 协商速率 | **Tx 2401 Mbps / Rx 1921 Mbps** |
| `ping 1.1.1.1` ×20 | **45% 丢包**，rtt 41–54 ms |
| 实际吞吐 | ≈10 KB/s |

链路层看起来完美，传输层几乎不通。原因在 dmesg 里：

```
ath11k_pci 0006:01:00.0: msdu_done bit in attention is not set
```

RX 描述符的 `msdu_done` 没置位，驱动只能把那一帧丢掉。

### 它是**负载触发**的，不是一直坏

统计很干净：整个 dmesg 里这条消息共 **25 次，全部落在 t=1423 s 到 t=1671 s**
之间，而当时 uptime 是 1690 s —— 也就是**只在下载那几分钟出现**，
开机后一直到下载开始为止一条都没有。而且它是**唯一一种** ath11k 报错。

⚠️ 一个诚实的相关性：出问题的这次跑在**内核 #19**（含 Venus 补丁），
而顺利的那次跑在 **#18**。我**没有**证据说 Venus 导致了它
（Venus 只加了一段 5 MB 保留内存和一个 IOMMU 上下文，与 ath11k 的 RX 路径无关），
而 `msdu_done` 在 ath11k 上是有名的间歇性问题。**记下这个相关性，但不要当成结论。**

### 绕过它的办法（本轮实测，很有用）

**不要让设备自己从网上下 OTA payload，改成 USB 推。** 实测三条链路的速度差异
非常大，值得记住：
...
| 设备 ↔ R2（WiFi，本次） | 10 KB/s（本缺陷）|

所以最快的路是 **R2 → 本机 → USB → 设备**，然后
`update_engine_client --payload=file:///data/local/tmp/payload.bin`。
实测 **76 秒**装完（走 WiFi 那次同样的包要二十多分钟）。
★ `update_engine` 支持 `file://`，这条在没有 recovery / 没有 sideload 的本机
是很实用的一手。

### 下一步

* 先确认能否在**内核 #18** 上复现（同样的大下载）—— 这能证否或证实上面那个相关性。
* 若与内核无关，就是上游 ath11k 的问题：查
  `ath11k_dp_rx_process_received_packets()` 里那句判断，看是否与
  cache 一致性 / `dma_sync` 有关。本机是 `ath11k_pci`，WCN6855。
* 用户日常影响未知（浏览网页不会持续打满 RX），但**任何大文件下载都会碰上**，
  包括系统内 OTA —— 所以这条对"用户自己升级"是有实际影响的。
